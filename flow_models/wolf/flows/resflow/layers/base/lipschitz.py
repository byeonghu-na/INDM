...
__all__ = ['SpectralNormLinear', 'SpectralNormConv2d', 'LopLinear', 'LopConv2d', 'get_linear', 'get_conv2d', 'get_convcond2d']
...
class LopCondConv2d(nn.Conv2d):
    ...
    def __init__(
        ...
        **unused_kwargs,
        ...
        del unused_kwargs
        super(LopCondConv2d, self).__init__(in_channels, out_channels, kernel_size, stride, padding, bias)
        self.name = name
        self.coeff = coeff
        self.domain = domain
        self.codomain = codomain
        ...
        max_across_input_dims, self.norm_type = operator_norm_settings(self.domain, self.codomain)
        self.max_across_dim = 1 if max_across_input_dims else 0
        self.register_buffer('scale', torch.tensor(0., dtype=torch.float32))
        ...
        h_channels = 64
        self.h_net = GlobalLinearCondNet(h_channels, in_channels)
        #####
    ...
    def forward(self, input, h=None):
        ...
        input = input + self.h_net(h)
        weight = self.compute_weight()
        out = F.conv2d(input, weight, self.bias, self.stride, self.padding, 1, 1)
        return out

    def extra_repr(self):
        s = super(LopCondConv2d, self).extra_repr()
        return s + ', coeff={}, domain={}, codomain={}, local={}'.format(
            self.coeff, self.domain, self.codomain, self.local_constraint
        )
...
def get_linear(in_features, out_features, bias=True, coeff=0.97, domain=None, codomain=None, name=None, **kwargs):
    ...
    if domain == 1:
        ...
        if domain in [2, float('inf')]:
            _linear = LopLinear
    return _linear(in_features, out_features, bias, coeff, domain, codomain, name=name, **kwargs)


def get_conv2d(
    in_channels, out_channels, kernel_size, stride, padding, bias=True, coeff=0.97, domain=None, codomain=None, name=None, **kwargs
):
    ...
    if domain == 1:
        ...
        if domain in [2, float('inf')]:
            _conv2d = LopConv2d
    return _conv2d(in_channels, out_channels, kernel_size, stride, padding, bias, coeff, domain, codomain, name=name, **kwargs)
