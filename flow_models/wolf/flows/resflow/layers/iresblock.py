...
import logging
from flow_models.wolf.flows.resflow.layers.base.lipschitz import LopCondConv2d
...
class iResBlock(nn.Module):
    ...
    def __init__(
        ...
        self.nnet = nnet
        self.n_dist = n_dist
        self.geom_p = nn.Parameter(torch.tensor(np.log(geom_p) - np.log(1. - geom_p), dtype=torch.float32))
    ...
    def nnet_forward(self, x, h):
        for net in self.nnet:
            if isinstance(net, LopCondConv2d):
                x = net(x, h=h)
            else:
                x = net(x)
        return x
    ...
    def forward(self, x, logpx=None, h=None):
        if logpx is None:
            y = x + self.nnet_forward(x, h=h)
            return y
        else:
            g, logdetgrad = self._logdetgrad(x, h=h)
            return x + g, logpx - logdetgrad

    def inverse(self, y, logpy=None, h=None):
        x = self._inverse_fixed_point(y, h=h)
        if logpy is None:
            return x
        else:
            return x, logpy + self._logdetgrad(x, h=h)[1]

    def _inverse_fixed_point(self, y, atol=1e-5, rtol=1e-5, h=None):
        x, x_prev = y - self.nnet_forward(y, h=h), y
        i = 0
        tol = atol + y.abs() * rtol
        while not torch.all((x - x_prev)**2 / tol < 1):
            x, x_prev = y - self.nnet_forward(x, h=h), x
            i += 1
            if i > 1000:
        ...
        with torch.enable_grad():
            ...
            if not self.exact_trace:
                ...
                if self.training and self.grad_in_forward:
                    raise NotImplementedError("Not working because of multiple inputs of nnet")
                    # g, logdetgrad = mem_eff_wrapper(
                    #     estimator_fn, self.nnet, x, n_power_series, vareps, coeff_fn, self.training, h=h
                    # )
                else:
                    x = x.requires_grad_(True)
                    g = self.nnet_forward(x, h=h)
...
class MemoryEfficientLogDetEstimator(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, estimator_fn, gnet, x, n_power_series, vareps, coeff_fn, training, h, *g_params):
...
def mem_eff_wrapper(estimator_fn, gnet, x, n_power_series, vareps, coeff_fn, training, h=None):
    ...
    return MemoryEfficientLogDetEstimator.apply(
        estimator_fn, gnet, x, n_power_series, vareps, coeff_fn, training, h, *list(gnet.parameters())
    )
