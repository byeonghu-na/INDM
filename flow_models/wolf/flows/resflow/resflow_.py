...
from flow_models.wolf.flows.resflow.layers import ActNorm2d, SequentialFlow, ActNorm1d, InvertibleLinear, InvertibleConv2d, iResBlock, MovingBatchNorm2d, SqueezeLayer, CouplingBlock, ChannelCouplingBlock, MaskedCouplingBlock
from flow_models.wolf.flows.resflow.layers.base import Swish, LipschitzCube, Identity, get_linear, get_conv2d, get_convcond2d, Sin
from models.layerspp import ResnetBlockBigGANpp
...
class ResidualFlow(nn.Module):
    ...
    def __init__(
        ...
        grad_in_forward=False, # True
        first_resblock = True,
        #first_resblock=False, # True
        learn_p=False,
        ...
        for i, hshape in enumerate(hidden_shapes):
            classification_heads.append(
                nn.Sequential(
                    ...
                    ActNorm2d(self.classification_hdim),
    ...
    def forward(self, x, logpx=None, h=None, reverse=False, classify=False):
        if reverse:
            return self.inverse(x, logpx, h=h)
        out = []
        if classify: class_outs = []
        for idx in range(len(self.transforms)):
            if logpx is not None:
                x, logpx = self.transforms[idx].forward(x, logpx, h=h)
            else:
                x = self.transforms[idx].forward(x, h=h)
    ...
    def inverse(self, z, logpz=None, h=None):
        if self.factor_out:
            ...
            if logpz is None:
                z_prev = self.transforms[-1].inverse(zs[-1], h=h)
                for idx in range(len(self.transforms) - 2, -1, -1):
                    z_prev = torch.cat((z_prev, zs[idx]), dim=1)
                    z_prev = self.transforms[idx].inverse(z_prev, h=h)
                return z_prev
            else:
                z_prev, logpz = self.transforms[-1].inverse(zs[-1], logpz, h=h)
                for idx in range(len(self.transforms) - 2, -1, -1):
                    z_prev = torch.cat((z_prev, zs[idx]), dim=1)
                    z_prev, logpz = self.transforms[idx].inverse(z_prev, logpz, h=h)
                return z_prev, logpz
        else:
            z = z.view(z.shape[0], *self.dims[-1])
            for idx in range(len(self.transforms) - 1, -1, -1):
                if logpz is None:
                    z = self.transforms[idx].inverse(z, h=h)
                else:
                    z, logpz = self.transforms[idx].inverse(z, logpz, h=h)
    ...
    def generate(self, x, h=None):
        ...
        if '-' in self.config.flow.nblocks:
            out = out.view(x.shape[0], x.shape[1], 2, 2, x.shape[2] // 2, x.shape[3] // 2)
            out = out.permute(0, 1, 4, 2, 5, 3).reshape(x.shape)
        logpx = logpx.squeeze(-1)

        return out, logpx
    '''
    def init(self, x, h=None):
        logpx = None
        out = []
        for idx in range(len(self.transforms)):
            if logpx is not None:
    ...
    def fwdpass(self, x: torch.Tensor, h, eval_logdet=True, init=False, init_scale=1.0, **kwargs):
        if eval_logdet:
            out, logpx = self.forward(x, logpx=0., h=h, reverse=False, classify=False)
        else:
            out = self.forward(x, logpx=None, h=h, reverse=False, classify=False)
        if '-' in self.config.flow.nblocks:
            out = out.view(x.shape[0], x.shape[1], 2, 2, x.shape[2] // 2, x.shape[3] // 2)
            out = out.permute(0, 1, 4, 2, 5, 3).reshape(x.shape)
        else:
    ...
    def bwdpass(self, x: torch.Tensor, h, init=False, init_scale=1.0, **kwargs):
        if init:
            self.init(x, h)
            return
        else:
            ...
            out = self.forward(x, logpx=None, h=h, reverse=True, classify=False)
            return out, None

class StackediResBlocks(SequentialFlow):
    ...
    def __init__(
        ...
        def _actnorm(size, fc):
            if fc:
                return FCWrapper(ActNorm1d(size[0] * size[1] * size[2]))
            else:
                return ActNorm2d(size[0])

        def _quadratic_layer(initial_size, fc):
            if fc:
                c, h, w = initial_size
                dim = c * h * w
                return FCWrapper(InvertibleLinear(dim))
            else:
                return InvertibleConv2d(initial_size[0])

        def _lipschitz_layer(fc):
            return get_linear if fc else get_conv2d
        def _lipschitz_layer_cond(fc):
            return None if fc else get_convcond2d

        def _resblock(initial_size, resblock_type, idim=idim, first_resblock=False):
            if resblock_type == 'fc':
                fc = True
                return iResBlock(
                    FCNet(
                ...
                if not first_resblock and preact:
                    if batchnorm: nnet.append(MovingBatchNorm2d(initial_size[0]))
                    nnet.append(ACT_FNS[activation_fn](False))

                nnet.append(
                    _lipschitz_layer(fc)(
                        initial_size[0], idim, ks[0], 1, ks[0] // 2, coeff=coeff, n_iterations=n_lipschitz_iters,
                        domain=_domains[0], codomain=_codomains[0], atol=sn_atol, rtol=sn_rtol, name=f'before'
                    )
                )
                if batchnorm: nnet.append(MovingBatchNorm2d(idim))
                nnet.append(ACT_FNS[activation_fn](True))
                for i, k in enumerate(ks[1:-1]):
                    nnet.append(
                        _lipschitz_layer_cond(fc)(
                            idim, idim, k, 1, k // 2, coeff=coeff, n_iterations=n_lipschitz_iters,
                            domain=_domains[i + 1], codomain=_codomains[i + 1], atol=sn_atol, rtol=sn_rtol, name=f'{i}_{k}'
                        )
                    )
                    if batchnorm: nnet.append(MovingBatchNorm2d(idim))
                    nnet.append(ACT_FNS[activation_fn](True))
                ...
                nnet.append(
                    _lipschitz_layer(fc)(
                        ...
                        domain=_domains[-1], codomain=_codomains[-1], atol=sn_atol, rtol=sn_rtol, name='after'
                    )
                )
                if batchnorm: nnet.append(MovingBatchNorm2d(initial_size[0]))
                return iResBlock(
        ...
        if squeeze:
            ...
            for i in range(n_blocks):
                ...
                chain.append(_resblock(initial_size, config.flow.resblock_type, first_resblock=first_resblock and (i == 0)))
            ...
            for _ in range(n_blocks):
                if quadratic: chain.append(_quadratic_layer(initial_size, fc))
                chain.append(_resblock(initial_size, config.flow.resblock_type))
                if actnorm: chain.append(_actnorm(initial_size, fc))
                if fc_actnorm: chain.append(_actnorm(initial_size, True))
            # Use four fully connected layers at the end.
            if fc_end:
                for _ in range(fc_nblocks):
                    chain.append(_resblock(initial_size, 'fc', fc_idim))
...
class FCWrapper(nn.Module):
    ...
    def forward(self, x, logpx=None, h=None):
        ...
        x = x.reshape(x.shape[0], -1)
        if logpx is None:
            y = self.fc_module(x, h=h)
            return y.view(*shape)
        else:
            y, logpy = self.fc_module(x, logpx, h=h)
    ...
    def inverse(self, y, logpy=None, h=None):
        ...
        if logpy is None:
            x = self.fc_module.inverse(y, h=h)
            return x.view(*shape)
        else:
            x, logpx = self.fc_module.inverse(y, logpy, h=h)
            return x.view(*shape), logpx


class StackedCouplingBlocks(SequentialFlow):
        ...
        def _resblock(initial_size, fc, idim=idim, first_resblock=False):
            if fc:
                ...
                if init_layer is None:
                    _block = ChannelCouplingBlock
                    _mask_type = 'channel'
                    div_in = 2
                    mult_out = 1
                else:
                    _block = MaskedCouplingBlock
