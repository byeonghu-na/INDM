...
from flow_models.wolf.modules import DeQuantizer
from flow_models.wolf.modules import Discriminator
from flow_models.wolf.modules import Generator
...
class WolfCore(nn.Module):
    ...
    def add_config(self, config):
        self.config = config
        self.generator.add_config(config)
    ...
    def synthesize(self, epsilon, nums, image_size=None, tau=1.0, n_bits=8, device=torch.device('cpu')):
        # [nsamples, imagesize]
        # epsilon = torch.randn(nums, *image_size, device=device)
        # epsilon *= tau
        z = self.discriminator.sample_from_prior(nums, device)
        imgs, _ = self.generator.flow.bwdpass(epsilon, h=z)
    ...
    def decode(self, epsilon, z=None, n_bits=8):
        ...
        return imgs
    '''
    def forward(self, data, y=None, n_bits=8, nsamples=1, reverse=False, eval_logdet=True):
        if reverse:
            z = self.discriminator.sample_from_prior(data.shape[0], data.device)
            if isinstance(self.generator.flow, ResidualFlow):
                imgs, _ = self.generator.flow.bwdpass(data, h=z)
            else:
                imgs, _ = self.generator.flow.fwdpass(data, h=z)
            # imgs = postprocess(imgs, n_bits=n_bits)
            return imgs
        else:
            ...
            if isinstance(self.generator.flow, ResidualFlow):
                if eval_logdet:
                    epsilon, logdet = self.generator.flow.fwdpass(x, h=z, eval_logdet=eval_logdet)
                else:
                    epsilon = self.generator.flow.fwdpass(x, h=z, eval_logdet=eval_logdet)
            else:
                epsilon, logdet = self.generator.log_probability(x, h=z)
                logdet = logdet.view(size[0], nsamples).mean(dim=1)
                logdet = logdet * -1.
            if eval_logdet:
                loss = - logdet - kl
                loss = loss.view(size[0], nsamples).mean(dim=1)
                return epsilon, loss
            else:
                return epsilon

    @classmethod
    def from_params(cls, params: Dict, config=None) -> "WolfCore":
        # discriminator
        ...
        generator = Generator.from_params(generator_params, config)

        return WolfCore(generator, discriminator, dequantizer)
