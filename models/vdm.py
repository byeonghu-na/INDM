...
class Noise_Schedule(nn.Module):
  def __init__(self):
    super(Noise_Schedule, self).__init__()
    self.dense_1 = torch.nn.Linear(1, 1)
    self.dense_2 = torch.nn.Linear(1, 1024)
    self.dense_3 = torch.nn.Linear(1024, 1)

  def forward(self, t):
    return self.dense_1(t) + self.dense_3(torch.sigmoid(self.dense_2(self.dense_1(t))))